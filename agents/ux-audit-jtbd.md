...
|---|---|---|---|
| `jtbd-coverage` | CRITICAL | spec | Every job has a path through the product, and every primary job has an obvious one. No gaps. |
| `journey-economy` | MAJOR | spec | Every step in the flow is hired by at least one job. No ceremony. |
...
Below `build`, the report leads with the jobs, and **you are the only agent that can supply
them**. Add a top-level `jobs` array beside `checks` and `findings`:
...
One entry per job in the frozen set, in order, including the ones that pass. A reader who
sees only failures cannot tell a broken feature from a thorough auditor.

`name` is the job in the user's terms, short enough to read in a table cell. `blocked_by` is
one clause naming the thing in the way, not a summary of your findings: "no state for a user
who has no resume yet", not "several problems with onboarding". Omit it when `served` is true.

`served` is your structural judgement: is there a working path through the product for this
