...
|---|---|---|
| `ux-no-dark-patterns` | CRITICAL | No forced continuity, roach motel, confirmshaming, hidden costs, or pre-ticked consent. Cancelling is as easy as subscribing. |
| `ux-consistency-standards` | MAJOR | One term per concept throughout. Same component behaves the same way everywhere. Platform conventions respected (logo top-left, search top-right, back button unbroken). |
| `ux-error-prevention` | MAJOR | Inputs are constrained rather than validated after the fact: pickers over free text, sensible defaults, autocomplete, unsaved-changes warnings. Slips and mistakes are prevented differently. |
| `ux-recognition-over-recall` | MAJOR | Nothing requires the user to remember something from a previous screen. Breadcrumbs, recent items, pre-filled fields, decoded values rather than codes. |
| `ux-minimalist-hierarchy` | MAJOR | One primary CTA per screen. Every element earns its place. What matters now is visible, the rest is progressively disclosed. |
| `ux-orientation-trunk-test` | MAJOR | Dropped on any screen, the user can answer: what product, what page, what are the sections, what are my options, where am I, where is search. Page titles match the link that was clicked. |
| `ux-scannable-not-thinkable` | MAJOR | Nothing on the screen makes the user stop and think "huh?". No walls of text, no mystery-meat icons without labels, no happy-talk. If a label needs explanation, the label is wrong. |
| `ux-flexibility-efficiency` | NICE_TO_HAVE | A repeat user has a faster path: shortcuts, bulk actions, saved state, a command palette. Progressive disclosure keeps the novice path simple. |
| `ux-contextual-help` | NICE_TO_HAVE | Help is task-focused and available in place (inline hints, tooltips), not only in a separate docs site. |
